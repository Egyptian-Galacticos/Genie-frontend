...
export class ChatService {
  ...
  private subscribeToWebSocketEvents(): void {
    this.webSocketService.newMessage$.subscribe((messageEvent: MessageEvent) => {
      if (messageEvent) {
        this.handleNewMessage(messageEvent)
      }
    })

    this.webSocketService.messageRead$.subscribe((readEvent: ReadEvent) => {
      if (readEvent) {
  ...
  private handleNewMessage(messageEvent: MessageEvent): void {
    const message: Message = {
      id: messageEvent.id,
      conversation_id: messageEvent.conversation_id,
      content: messageEvent.content,
      type: messageEvent.type as "text" | "image" | "file" | "audio" | "video",
      sent_at: messageEvent.sent_at,
      created_at: messageEvent.created_at,
      updated_at: messageEvent.updated_at || messageEvent.created_at,
      is_read: false,
  ...
  private handleMessageRead(readEvent: ReadEvent): void {
    this._messages.update((messagesMap) => {
      const newMap = new Map(messagesMap)
  ...
  loadConversations(params?: ConversationListParams): Observable<Conversation[]> {
    ...
    return this.http.get<ConversationResponse>(`${this.baseUrl}/conversations`, { params: httpParams }).pipe(
      map((response) => {
        if (response.success) {
  ...
  loadMessages(conversationId: number, page = 1, per_page = 50): Observable<Message[]> {
    ...
    const params = new HttpParams().set("page", page.toString()).set("per_page", per_page.toString())

    return this.http.get<MessageResponse>(`${this.baseUrl}/conversations/${conversationId}/messages`, { params }).pipe(
      map((response) => {
        if (response.success) {
          const messages = response.data.sort((a, b) => new Date(a.sent_at).getTime() - new Date(b.sent_at).getTime())
  ...
  sendMessage(conversationId: number, content: string, type = "text"): Observable<Message> {
    ...
    return this.http
      .post<ApiResponse<Message>>(`${this.baseUrl}/conversations/${conversationId}/messages`, payload)
      .pipe(
        map((response) => {
          if (response.success) {
  ...
  searchConversations(query: string): Observable<Conversation[]> {
    ...
    return this.http.get<ConversationResponse>(`${this.baseUrl}/conversations/search`, { params }).pipe(
      map((response) => {
        if (response.success) {
          return response.data
        }
  ...
  markConversationAsRead(conversationId: number): Observable<void> {
    return this.http.patch<ApiResponse<void>>(`${this.baseUrl}/conversations/${conversationId}/read`, {}).pipe(
      tap(() => {
        this._conversations.update((conversations) =>
          conversations.map((conv) => {
            if (conv.id === conversationId) {
